...
#include <sys/select.h> // --- NEW: For FD_SETSIZE, a common max FD limit ---
#include <sys/socket.h> // For socket, bind, listen, accept functions
#include <time.h>       // --- NEW: For time() function to get current time ---
...
// --- NEW: Define the Keep-Alive idle timeout in seconds ---
#define KEEPALIVE_IDLE_TIMEOUT_SECONDS                                         \
  5 // Close connection if idle for 5 seconds
...
typedef enum {
  READING_REQUEST,
  WRITING_RESPONSE,
  // CLOSING_CONNECTION is handled implicitly by returning 1 from handler
  // functions indicating the connection should be closed.
} client_state_enum;
...
typedef struct {
  ...
  int keep_alive;         // Flag to indicate if keep-alive is enabled for this
                          // connection (0 or 1)
  time_t last_activity_time; // --- NEW: Timestamp of the last activity on this
                             // connection ---
} client_state_t;

// --- NEW: Global array to map file descriptors to client state structs ---
// FD_SETSIZE is a common system-defined limit for file descriptors (often
// 1024). This allows us to quickly find a client's state by its FD.
...
void handle_new_connection(int listen_sock, int epoll_fd) {
  ...
  while (1) {
    ...
    if (active_clients_count >= MAX_ACTIVE_CLIENTS) {
      printf(
          "Client limit reached (%d). Rejecting new connection on socket %d.\n",
    ...
    client_state->last_activity_time =
        time(NULL); // --- NEW: Set initial activity time ---

    // Add the new client socket to the epoll instance.
    // We want to be notified when this client socket has data to read
    // (EPOLLIN). We will add EPOLLOUT later when we have a response to send.
    // epoll_event.data.ptr now points to our client_state_t
    event.events = EPOLLIN | EPOLLET; // Watch for input, edge-triggered
    event.data.ptr =
        client_state; // Associate the event with our client_state_t
    ...
    // --- NEW: Store client state in the global map ---
    if (conn_sock < FD_SETSIZE) { // Ensure FD is within our map bounds
      client_states_map[conn_sock] = client_state;
    } else {
      fprintf(stderr,
              "Warning: Client FD %d exceeds FD_SETSIZE. Cannot track for "
              "timeout.\n",
              conn_sock);
      // In a real server, you'd handle this more robustly (e.g., close
      // connection, use a different map)
    }
  }
}
...
int handle_read_event(client_state_t *client_state, int epoll_fd) {
  ...
  while (1) {
    ...
    bytes_transferred =
        read(current_fd, client_state->in_buffer + client_state->in_buffer_len,
    ...
    if (bytes_transferred == -1) {
      ...
      printf("Received %zd bytes from client %d. Total: %zu. Data: '%s'\n",
             bytes_transferred, current_fd, client_state->in_buffer_len,
             client_state->in_buffer);

      client_state->last_activity_time =
          time(NULL); // --- NEW: Update activity time on read ---
      ...
      if (strstr(client_state->in_buffer, "\r\n\r\n") != NULL) {
        printf(
            "Full HTTP request received from client %d. Preparing response.\n",
            current_fd);
        ...
        new_event.events =
            EPOLLOUT | EPOLLET; // Now primarily interested in writing
        ...
        break; // Exit read loop, as we've processed the request and changed
               // state
      }

      // If buffer is full and no full request, it's an error or too large
      // request
      if (client_state->in_buffer_len >=
          (sizeof(client_state->in_buffer) - 1)) {
...
int handle_write_event(client_state_t *client_state, int epoll_fd) {
  ...
  if (remaining_to_send > 0) {
    ...
    if (bytes_transferred == -1) {
      if (errno == EAGAIN || errno == EWOULDBLOCK) {
        // Kernel send buffer is full, try again later when EPOLLOUT fires
        // again.
      ...
      client_state->last_activity_time =
          time(NULL); // --- NEW: Update activity time on write ---
      ...
      if (client_state->out_buffer_sent >= client_state->out_buffer_len) {
        ...
        if (client_state->keep_alive) {
          ...
          struct epoll_event new_event;
          new_event.events =
              EPOLLIN | EPOLLET; // Now interested in reading again
      ...
      // If not all sent, EPOLLOUT will fire again (due to EPOLLET) when more
...
void close_client_connection(int epoll_fd, client_state_t *client_state) {
  ...
  // already implicitly removed or never added, which is not a critical error
  // here.
  if (epoll_ctl(epoll_fd, EPOLL_CTL_DEL, current_fd, NULL) == -1 &&
      errno != ENOENT) {
    perror("epoll_ctl: DEL client socket failed"); // Only print if it's a real
                                                   // error
  }
  close(current_fd); // Now close the socket itself.

  // --- NEW: Remove client state from the global map before freeing ---
...
void handle_client_event(int epoll_fd, struct epoll_event *event_ptr) {
  ...
  // Check for connection closed by peer (EPOLLRDHUP), hang up (EPOLLHUP), or
  // error (EPOLLERR)
  ...
  // If any of the handlers or initial checks determined the connection should
  // be closed
  if (should_close) {
...
int main() {
  ...
  // --- NEW: Initialize the client_states_map to NULL ---
  ...
  event.events =
      EPOLLIN | EPOLLET; // Watch for new connections (input), edge-triggered
  event.data.fd =
      listen_sock; // Associate this event with the listening socket's FD
  ...
  while (1) {
    ...
    // can periodically check for Keep-Alive timeouts even if no I/O events
    // occur.
    num_events = epoll_wait(epoll_fd, events, MAX_EVENTS,
                            1000); // Wait for up to 1 second
    if (num_events == -1) {
      // If epoll_wait was interrupted by a signal (e.g., Ctrl+C), just
      // continue.
      if (errno == EINTR) {
        continue;
      }
    ...
    for (i = 0; i < num_events; i++) {
      ...
      if (events[i].data.fd == listen_sock) {
        ...
        // Pass the entire epoll_event structure, as it contains our
        // client_state_t pointer.
        handle_client_event(epoll_fd, &events[i]);
      }
    }

    // --- NEW: Check for Keep-Alive timeouts after processing I/O events ---
    // Iterate through all possible file descriptors to find active clients.
    // This is simple but can be inefficient if FD_SETSIZE is very large and FDs
    // are sparse. For MAX_ACTIVE_CLIENTS = 100, this is perfectly fine.
    time_t current_time = time(NULL);
    for (i = 0; i < FD_SETSIZE; i++) {
      client_state_t *client_state = client_states_map[i];
      if (client_state != NULL && client_state->keep_alive) {
        // If the client is in READING_REQUEST state and has been idle for too
        // long
        if (client_state->state == READING_REQUEST &&
            (current_time - client_state->last_activity_time) >
                KEEPALIVE_IDLE_TIMEOUT_SECONDS) {
          printf("Client socket %d (Keep-Alive) timed out after %d seconds "
                 "idle. Closing.\n",
